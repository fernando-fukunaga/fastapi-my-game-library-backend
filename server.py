...
@app.post("/usuarios", response_model=schemas_usuario.UsuarioDadosSimples, status_code=201)
async def cadastar_usuarios(usuario: schemas_usuario.UsuarioCadastro, banco_de_dados: Session = Depends(get_banco_de_dados)):
...
async def remover_usuario(id_usuario: int, banco_de_dados: Session = Depends(get_banco_de_dados)):
    return RepositorioUsuario(banco_de_dados).remover(id_usuario)

#================================================================================================================

@app.post("/plataformas", response_model=schemas_plataforma.PlataformaDadosSimples, status_code=201)
async def cadastrar_plataformas(plataforma: schemas_plataforma.PlataformaCadastro, banco_de_dados: Session = Depends(get_banco_de_dados)):
...
async def listar_plataformas(banco_de_dados: Session = Depends(get_banco_de_dados)):
...
async def obter_plataforma(id_plataforma: int, banco_de_dados: Session = Depends(get_banco_de_dados)):
    return RepositorioPlataforma(banco_de_dados).obter(id_plataforma)

@app.delete("/plataformas/{id_plataforma}")
async def remover_plataforma(id_plataforma: int, banco_de_dados: Session = Depends(get_banco_de_dados)):
    return RepositorioPlataforma(banco_de_dados).remover(id_plataforma)

#================================================================================================================

@app.post("/jogos", response_model=schemas_jogo.JogoDadosSimples, status_code=201)
async def cadastrar_jogos(jogo: schemas_jogo.JogoCadastro, banco_de_dados: Session = Depends(get_banco_de_dados)):
    return RepositorioJogo(banco_de_dados).criar(jogo)

@app.get("/jogos", response_model=List[schemas_jogo.JogoDadosSimples])
async def listar_jogos(banco_de_dados: Session = Depends(get_banco_de_dados)):
    return RepositorioJogo(banco_de_dados).listar()

@app.get("/jogos/{id_jogo}", response_model=schemas_jogo.JogoDadosSimples)
async def obter_jogo(id_jogo: int, banco_de_dados: Session = Depends(get_banco_de_dados)):
    return RepositorioJogo(banco_de_dados).obter(id_jogo)

@app.delete("/jogos/{id_jogo}")
async def remover_jogo(id_jogo: int, banco_de_dados: Session = Depends(get_banco_de_dados)):
